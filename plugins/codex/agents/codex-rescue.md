...
- `--background` is a Claude-side flag — strip it before invoking `task`. `--wait` is forwarded through to `task` so the companion handles the detached poll-and-return flow.
